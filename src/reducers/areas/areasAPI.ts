...
export const createAreaAPI = async (formData: { name: string, description: string, application_id: number }): Promise<Area> => {
    const response = await apiClient.post<Area>('/areas', { ...formData });
    return response.data;
};

export const updateAreaAPI = async (id: number, formData: { name: string, description: string, application_id: number }): Promise<Area> => {
    const response = await apiClient.put<Area>(`/areas/${id}`, { ...formData });
    return response.data;
};

export const deleteAreaAPI = async (ids: number[]): Promise<void> => {
    await apiClient.delete(`/areas`, { data: { ids } });
};
