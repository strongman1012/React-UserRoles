...
interface ModulationModalProps<T extends { Name: string } = { Name: string }> {
	modulations: T[];
	open: boolean;
	onOpen: () => void;
...
const ModulationModal: FC<ModulationModalProps> = ({
	modulations,
	open,
	onOpen,
			...
			<Formik
				...
				validationSchema={Yup.object().shape({
					name: Yup.string()
						...
						// using Yup to check name field duplication using modulations array object
						.test('uniqueName', 'Name already exists', (value) => {
							return !modulations.some(
								(modulation) => modulation.Name === value
							);
						})
				})}
				onSubmit={async (
					...
					try {
						await handleCreateModulation(values);
